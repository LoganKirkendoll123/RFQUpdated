/*
  ...
  1. New Tables
    ...
    - `verification_codes`
      ...
      - `email` (text)
      - `code` (text)
      - `code_type` (text, email_verification/password_reset)
...
-- Create admin account using proper upsert
DO $$
DECLARE
  admin_user_id uuid;
BEGIN
  -- First, try to create or get the auth user
  ...
  -- Get the user ID if the insert was skipped due to conflict
  IF admin_user_id IS NULL THEN
    SELECT id INTO admin_user_id FROM auth.users WHERE email = 'lkirkendoll@fitzmark.com';
  END IF;

  -- Now create or update the user profile using proper upsert
  ...
  RAISE NOTICE 'Admin account setup completed for lkirkendoll@fitzmark.com with user_id: %', admin_user_id;
END $$;