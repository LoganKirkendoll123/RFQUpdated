...
export const MarginAnalysisTools: React.FC = () => {
  ...
  const loadCustomerListFromShipments = async () => {
    try {
      console.log('👥 Loading customer list from shipment history...');
      ...
      if (startDate) {
        query = query.gte('"Scheduled Pickup Date"', startDate);
      }
      
      if (endDate) {
        query = query.lte('"Scheduled Pickup Date"', endDate);
      }
      
      const { data: customerData, error: customerError } = await query;
      
      if (customerError) {
        console.error('❌ Error loading customers from shipments:', customerError);
        throw customerError;
      }
      
      const uniqueCustomers = [...new Set(customerData?.map(d => d.Customer).filter(Boolean) || [])];
      setCustomerList(uniqueCustomers);
      console.log(`✅ Loaded ${uniqueCustomers.length} unique customers from shipment history`);
      
    } catch (err) {
      console.error('❌ Failed to load customer list from shipments:', err);
      // Don't throw error here, just log it - this is not critical for the main functionality
    }
  };
