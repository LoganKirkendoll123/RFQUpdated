...
export const MarginAnalysisTools: React.FC = () => {
  ...
  const loadJobs = async () => {
    try {
      console.log('🔍 Loading margin analysis jobs...');
      ...
      if (queuedError) {
        console.error('❌ Error loading queued jobs:', queuedError);
      } else {
        setQueuedJobs(queuedData || []);
        console.log(`✅ Loaded ${queuedData?.length || 0} queued jobs`);
      }
      ...
      if (completedError) {
        ...
        console.log(`✅ Loaded ${completedData?.length || 0} completed jobs`);
      }
    } catch (err) {
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      // Get shipment count for the selected criteria
      const { count, error: countError } = await supabase
        .from('Shipments')
        .select('*', { count: 'exact', head: true })
        .eq(selectedCustomer !== 'All Customers' ? 'Customer' : 'Customer', selectedCustomer !== 'All Customers' ? selectedCustomer : 'Customer')
      ...
      console.log(`✅ Found ${shipmentCount} shipments matching criteria`);
      ...
      console.log(`✅ Analysis job created: ${data.id} with ${shipmentCount} shipments`);
  ...
  const runAnalysis = async (jobId: string) => {
    try {
      ...
      console.log(`✅ Retrieved ${shipments?.length || 0} shipments for analysis`);
      
      // Process shipments in batches to show progress
      const totalShipments = shipments?.length || 0;
      for (let i = 0; i < totalShipments; i += 10) {
        const progress = Math.round((i / totalShipments) * 100);
      ...
      // Get real results from the database
              ...
              <div key={index} className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Building2 className="h-4 w-4 text-gray-400" />
                      <span className="text-sm text-gray-700">{result.customer_name || 'Unknown Customer'}</span>
                    ...
                    <div className="flex items-center space-x-2">
                      <Truck className="h-4 w-4 text-gray-400" />
                      <span className="text-sm text-gray-700">{selectedCarrier || result.carrier_name}</span>
                    </div>
