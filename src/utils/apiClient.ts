...
export class Project44APIClient {
  ...
  async getQuotes(
    ...
    // Add totalLinearFeet for VLTL requests (required field) - NO enhanced handling units
    if (isVolumeMode) {
      requestPayload.totalLinearFeet = rfq.totalLinearFeet || this.calculateLinearFeet(rfq);
      console.log(`📏 Using totalLinearFeet: ${requestPayload.totalLinearFeet} for VLTL request (no enhanced handling units)`);
    }
