...
// Enhanced handling unit interface for VLTL
interface EnhancedHandlingUnit {
  description: string;
...
export class Project44APIClient {
  ...
  private buildLineItems(rfq: RFQRow): LineItem[] {
    ...
    const lineItem: LineItem = {
      ...
      id: '1',
      insuranceAmount: rfq.insuranceAmount
    };
    ...
    console.log(`📦 Built single line item from RFQ: ${lineItem.description} - ${lineItem.totalWeight}lbs - ${lineItem.freightClass}`);
    return [lineItem];
  }

  private buildAccessorialServices(rfq: RFQRow, isReeferMode: boolean = false): AccessorialService[] {
    const accessorials: AccessorialService[] = [];

    // Add temperature control for reefer mode
    if (isReeferMode && rfq.temperature) {
      accessorials.push({
        code: 'TEMP_CONTROL',
      ...
      console.log(`🌡️ Added temperature control accessorial: ${rfq.temperature}°F`);
    }

    // Add other accessorials based on RFQ data
    if (rfq.liftgateRequired) {
      accessorials.push({
        code: 'LIFTGATE',
    ...
    console.log(`🔧 Built ${accessorials.length} accessorial services:`, accessorials.map(a => a.code));
    return accessorials;
  }

  private buildPickupWindow(rfq: RFQRow): TimeWindow | undefined {
    if (!rfq.fromDate) return undefined;

    const pickupDate = new Date(rfq.fromDate);
    
    return {
      startDateTime: pickupDate.toISOString(),
      endDateTime: new Date(pickupDate.getTime() + 24 * 60 * 60 * 1000).toISOString() // Add 24 hours
    };
  }

  private buildDeliveryWindow(rfq: RFQRow): TimeWindow | undefined {
    if (!rfq.toDate) return undefined;

    const deliveryDate = new Date(rfq.toDate);
    
    return {
      startDateTime: deliveryDate.toISOString(),
      endDateTime: new Date(deliveryDate.getTime() + 24 * 60 * 60 * 1000).toISOString() // Add 24 hours
    };
  }
}