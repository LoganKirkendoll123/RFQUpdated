/*
  ...
  1. New Tables
    ...
    - `carriers`
      - `id` (uuid, primary key) 
      - `name` (text, unique)
      - `scac`, `mc_number`, `dot_number` fields

  2. Data Migration
    - Extract unique customers from CustomerCarriers.InternalName
    - Extract unique carriers from CustomerCarriers.P44CarrierCode and Shipments carriers
    - Clear existing invalid UUID references
    - Update foreign key columns with proper references
    - Add foreign key constraints

  3. Security
    - Enable RLS on both tables
    - Add policies for authenticated users (read) and admins (write)
*/
