/*
  # Fix Carrier Migration with Proper Type Handling

  1. New Tables
    - Add account_code column to carriers table
  
  2. Data Migration
    - Migrate carriers from CustomerCarriers and Shipments tables
    - Handle proper type conversion from bigint to text
    - Update foreign key relationships
  
  3. Security
    - Maintain existing RLS policies on carriers table
*/
...
BEGIN
  ...
  FOR carrier_record IN 
    ...
    AND "Quoted Carrier" != ''
    AND "Quoted Carrier" NOT IN (
      SELECT DISTINCT "Booked Carrier" 
      FROM "Shipments" 
      WHERE "Booked Carrier" IS NOT NULL
    )
  LOOP
