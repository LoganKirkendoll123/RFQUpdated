/*
  # Add P44 Account Code to Carriers

  1. Changes
    - Add account_code column to carriers table
    - Create index for account_code lookups
    - Update migration function to populate account_code from CustomerCarriers data
    - Convert CarrierId from bigint to text for account_code storage
...
BEGIN
  ...
  FOR carrier_record IN 
    ...
    AND "Booked Carrier" != ''
    AND "Booked Carrier" NOT IN (SELECT name FROM carriers WHERE name IS NOT NULL)
  LOOP
    -- Insert carrier if not exists
  ...
  FOR carrier_record IN 
    ...
    AND "Quoted Carrier" != ''
    AND "Quoted Carrier" NOT IN (SELECT name FROM carriers WHERE name IS NOT NULL)
  LOOP
    -- Insert carrier if not exists
    INSERT INTO carriers (name)
    VALUES (carrier_record.name)
