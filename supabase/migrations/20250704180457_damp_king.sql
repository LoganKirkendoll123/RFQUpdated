/*
  # Fix user sessions and policies

  1. New Functions
    - `handle_new_auth_session`: Tracks user sessions in the user_sessions table
    - `update_user_last_login`: Updates the last_login field in user profiles
    - `update_session_last_activity`: Updates the last_activity timestamp on session updates
  
  2. Triggers
    - Create triggers on auth.sessions for session tracking
    - Create trigger on user_sessions for activity tracking
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_sessions' 
    AND policyname = 'Service role can manage sessions'
  ) THEN
