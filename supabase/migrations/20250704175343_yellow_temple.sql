/*
  # Fix session logging functions and policies
  
  1. New Functions
    - `handle_new_auth_session`: Captures session data when a user logs in
    - `update_user_last_login`: Updates the last_login field in user_profiles
    - `update_session_last_activity`: Updates the last_activity timestamp
  ...
  3. Changes
    - Fixed policies to use auth.uid() instead of uid()
*/
